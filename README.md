...
- **即时正向反馈**（动画 + 音效）：
  ...
  - 节奏偏慢 → 加油鼓励「你可以的！💪」+ 轻快旋律
  - 满整十报数只报数字；每 5 个里程碑语音播报；暂停时计时同步停止
- **离线语音包（跨设备一致音色）**：87 个 wav 打进 `assets/voice/`（SHA-1 命名），静态鼓励语 + 动态句「前缀+逐字报数+后缀」MediaPlayer 顺序拼接，任意手机音色一致；缺失时回退系统 TTS（高音高童声兜底）。语料单一来源 `audio/Encouragements.kt`，生成脚本 `tools/gen_voice_pack.py`、校验脚本 `tools/verify_voice_assets.py`。
- **童音音效**：6 个 `res/raw/snd_*.wav`（TTS+音调上移+钟声层生成，脚本 `tools/gen_sound_effects.py`），SoundPlayer 优先播放、缺失回退实时合成。
- **积分与奖励系统**：每跳 +1 分、每满 10 个弹幕 +10、连跳 20/50/100 加成、达成每日目标 +50；9 档奖品墙（已兑换/未解锁状态），按 childId 隔离存储；小奖品从屏幕随机位置冒出。
- **我的页**：孩子彩色主页卡（头像/昵称/等级徽章/切换）+ 积分等级进度 + 奖品墙 + 常用入口（运动报告/成就墙/孩子管理/设置）+ 版本信息。
- **设置页（独立）**：训练设置（计数方式/灵敏度/每日每周目标）、每日提醒（开关+时间）、声音与语音（音效/语音/夜间模式/语音包下载/离线优先）、恢复默认；入口在「我的」页，首页「调整目标」直达。
- **每日提醒**：WorkManager 周期通知，设置里可开关并选择时间（Android 13+ 会申请通知权限）。
- **数据统计**：最近 7 天柱状图（含本周总数）、本周目标进度（附上周对比）、累计个数、最佳单次、打卡天数、连续天数、累计时长、消耗热量。
- **历史记录**：按天列表，删除需二次确认。
- **成就墙**：10 个关卡式徽章，点亮解锁；进度卡含「下一枚」提示；解锁弹庆祝 + 头像化头部。
- **首页**：问候 + 上次成绩摘要卡（点击跳统计）+ 本周打卡日历（每天可点查看当日个数）+ 今日目标星级进度；达标后按钮变庆祝文案。
